...
var selectOptions = ["#веловоскресенье"];
...
function initMultiSelect() {
  ...
  getContentCardDataTags().forEach(function (item) {
...
function getContentCardDataTags() {
  ...
  for (var i = 0; i < contentCards.length; i++) {
    var contentCard = contentCards[i];
    var contentCardTags = contentCard.dataset.tags.split(",");
    tags.push.apply(tags, _toConsumableArray(contentCardTags)); //спреад-оператор: передаём все элементы массива
  }
  var transformedTags = [];
